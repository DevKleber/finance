  ...
  intercept(
    ...
    return next.handle(request).pipe(
      catchError((errorResponse: HttpErrorResponse) => {
        ...
        if (
          (errorResponse.status === 401 && error.error === "token_expired") ||
          ...
          //     .pipe(
          //         flatMap(data => {
          //             localStorage.setItem("dG9rZW5maW5hbmNl", data.token)
          //             const authRequest = request.clone({ setHeaders: { 'Authorization': `Bearer ${data.dG9rZW5maW5hbmNl}` } })
