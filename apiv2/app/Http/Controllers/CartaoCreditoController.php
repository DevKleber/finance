        ...
        foreach ($getFaturasCartao as $key => $value) {
            ...
            if ($faturas[$key]['pessoas']->count() > 0) {
                foreach ($faturas[$key]['pessoas'] as $keyAmigo => $valueAmigo) {
                    ...
                    $dividaAmigos[$valueAmigo->id_pessoa]['categoria'][$value->no_categoria_despesa] = $dividaAmigos[$valueAmigo->id_pessoa]['valor'];
                }
            } else {
                $dividaAmigos[$usuarioLogado->id_pessoa]['valor'] =
                    isset($dividaAmigos[$usuarioLogado->id_pessoa]['valor']) ?
                        ...
                        $value->vl_despesa;
                $dividaAmigos[$usuarioLogado->id_pessoa]['categoria'][$value->no_categoria_despesa] = $dividaAmigos[$usuarioLogado->id_pessoa]['valor'];
            }
        }
