...
class AguardandoAprovacaoController extends Controller
{
    ...
    {
        $pessoa = \JWTAuth::parseToken()->authenticate()['id_pessoa'];
    ...
    {
        $compartilhada = \App\DespesaCompartilhada::where('id_conta_compartilhada', $request['id_conta_compartilhada'])->where('id_pessoa', \JWTAuth::parseToken()->authenticate()['id_pessoa'])->first();
        if ($compartilhada) {
